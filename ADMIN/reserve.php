...
<li>${highlightText(item.product_name, searchInput.value)} -
    ...
    ${
    item.stock <= 0 ? '<span class="out-of-stock">Out of Stock</span>' : "" } </li>`
        )
        .join("");
        ...
        cartItemsList.innerHTML = "
<li>No items found for this member.</li>";
}
...
const isOutOfStock = product.stock <= 0; const productHtml=` <div class="product-box" data-product-name="${
              product.product_name
            }">
    <div>${product.product_name} - P${product.price}</div>
    <div>Stock: ${
        product.stock <= 0 ? "Out of Stock" : product.stock }</div>
            <button class="add-item-button" onclick="addToCart(${product.id})" ${ isOutOfStock
    ...
    if (stock <= 0) { alert(`The product "${productName}" is out of stock.`); return; // Prevent adding to cart } if
        (quantity> stock) {
        alert(
        ...
        <button style='background-color:transparent;margin-top:-10px;color:red;' onclick="removeCartItem(${index})"
        ...
        Swal.fire({
        html: `<img src="../Assets/success_message.png"
            style="width: 50px; height: 50px; display: block; margin: 0 auto;" alt="Success Check"> Order added
        successfully!`,
